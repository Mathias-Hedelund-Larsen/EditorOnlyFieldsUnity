...
#pragma warning disable 0649
        ...
        [SerializeField]
        private Internal.EditorField[] _allFields;
...
#pragma warning restore 0649
        ...
        public string FindGUID(string sceneGuid, int objectID)
        {
            var allFieldsSearch = _allFields.Where(f => f.SceneGuid == sceneGuid && f.ObjectID == objectID).ToArray();

            if(allFieldsSearch.Length > 0)
            {
                var found = Array.Find(allFieldsSearch, f => !string.IsNullOrEmpty(f.GuidPath));

                if(found != null)
                {
                    return found.GuidPath;
                }                
            }
